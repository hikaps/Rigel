...
final class RigelHlsExporter {
    ...
    private static func run(
        ...
        for i in 0..<inCount {
            ...
            case AVMEDIA_TYPE_VIDEO:
                // REMUX may stream-copy H.264. TRANSCODE is an explicit
                // normalization request from FormatRouter (Hi10P/4:2:2/etc.);
                // copying the original bitstream there would fail again.
                if mode == "remux", codecpar.pointee.codec_id == AV_CODEC_ID_H264 {
        ...
        while true {
            ...
            if let outIdx = streamMap[inIdx],
               ...
               let outStream = out.pointee.streams[Int(outIdx)] {
                ...
                if let error = videoChains.values.compactMap(\.error).first {
                    terminalError = error
                }
                if !notified && terminalError == nil &&
    ...
    private final class VideoChain {
        ...
        init(
            ...
            self.frameDurationPTS = frameDurationPTS
            self.decCtx = decCtx
            self.encCtx = encCtx
            self.bsf = bsf
    ...
    private static func makeVideoChain(
        ...
        let frameRate = sourceFrameRate(avg: advertisedRate, nominal: fallbackRate)
        let frameDurationPTS = max(
            1,
            Int64((Double(encoderTimeBase.den) / Double(encoderTimeBase.num) / frameRate).rounded())
        )
        ...
        allocatedEnc.pointee.max_b_frames = 0
        ...
        return VideoChain(
            ...
            frameDurationPTS: frameDurationPTS,
            decCtx: decCtx,
            encCtx: allocatedEnc,
            bsf: bsf
        )
    }
    private static func nextVideoPTS(
        chain: VideoChain,
        frame: UnsafeMutablePointer<AVFrame>
    ) -> Int64 {
        let rawPTS = frame.pointee.best_effort_timestamp != Int64.min
            ...
            : frame.pointee.pts
        let candidate = rawPTS == Int64.min
            ? nil
            : av_rescale_q(rawPTS, chain.inTimeBase, chain.encCtx.pointee.time_base)
        let result = repairVideoPTS(
            candidate: candidate,
        ...
        chain.lastVideoPTS = result.repaired
        chain.videoPTSOffset = result.offset
        chain.lastRawVideoPTS = result.lastRaw
    ...
    private static func writeTranscodedVideo(
        ...
        let ret = avcodec_send_packet(chain.decCtx, packet)
        guard ret >= 0 else {
            let message = "video decoder rejected packet: \(avErrorString(ret))"
            NSLog("[RigelHls] %@", message)
            chain.error = message
            return
        }
        var decoded: UnsafeMutablePointer<AVFrame>? = av_frame_alloc()
    ...
    private static func convertVideoFrame(
        ...
        guard av_frame_get_buffer(dst, 0) >= 0 else {
            chain.error = "converted video frame allocation failed"
            NSLog("[RigelHls] %@", chain.error!)
            return
        }
        ...
        guard let activeSws = cachedSws else {
            chain.error = "video scaler setup failed"
            NSLog("[RigelHls] %@", chain.error!)
            av_frame_unref(dst)
            return
        }
        ...
        dst.pointee.pts = nextVideoPTS(chain: chain, frame: src)
        let ret = avcodec_send_frame(chain.encCtx, dst)
        if ret < 0 {
            chain.error = "H.264 encoder rejected frame: \(avErrorString(ret))"
            NSLog("[RigelHls] %@", chain.error!)
        } else {
    ...
    // MARK: - Timing and priming helpers
    ...
    static func repairVideoPTS(
        ...
        guard let previousRepaired else {
            ...
            if let raw = candidate { return (repaired, repaired - raw, raw) }
    ...
    static func audioRingHeadPTS90k(
        ...
        for packet in packets {
            ...
            if timestamp != Int64.min {
                return av_rescale_q(timestamp - durationBefore, timeBase, AVRational(num: 1, den: 90_000))
            }
    ...
    static func classifyPrimingRead(_ ret: Int32) -> PrimingReadResult {
        if ret >= 0 { return .ok }
        if ret == -541_478_725 { return .eof }
        if ret == -EAGAIN { return .again }
